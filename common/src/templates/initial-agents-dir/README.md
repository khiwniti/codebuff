...
2. **Test your agent**: Run `openbuff` without `--agent` so local `.agents` load, then invoke your agent from a prompt
3. **Publish your agent**: Run `openbuff publish your-agent-name`

## Need Help?

- For examples, check the `examples/` directory.
- Join our [Discord community](https://openbuff.com/discord) and ask your questions!
- Check our [documentation](https://openbuff.com/docs) for more details

# What is Openbuff?

Openbuff is an **open-source AI coding assistant** that edits your codebase through natural language instructions. Instead of using one model for everything, it coordinates specialized agents that work together to understand your project and make precise changes.

Openbuff beats Claude Code at 61% vs 53% on [our evals](https://github.com/OpenbuffAI/openbuff/tree/main/evals) across 175+ coding tasks over multiple open-source repos that simulate real-world tasks.
...
**Reuse any published agent**: Compose existing [published agents](https://www.openbuff.com/store) to get a leg up. Openbuff agents are the new MCP!
...
1. **Local Testing**: Run `openbuff` without `--agent` so local `.agents` load, then invoke your agent from a prompt
...
3. **Publish**: `openbuff publish your-agent-name`
...
## Why Choose Openbuff for Custom Agents
...
**Fully customizable SDK**: Build Openbuff's capabilities directly into your applications with a complete TypeScript SDK. Create custom tools, integrate with your CI/CD pipeline, build AI-powered development environments, or embed intelligent coding assistance into your products.

Learn more about the SDK [here](https://www.npmjs.com/package/@openbuff/sdk).

## Community & Support

- **Discord**: [Join our community](https://openbuff.com/discord) for help and inspiration
- **Examples**: Study the `examples/` directory for patterns
- **Documentation**: [openbuff.com/docs](https://openbuff.com/docs) and check `types/` for detailed type information
- **Issues**: [Report bugs and request features on GitHub](https://github.com/OpenbuffAI/openbuff/issues)
- **Support**: [support@openbuff.com](mailto:support@openbuff.com)
