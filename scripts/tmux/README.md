...
- **Works with any TUI app** - Openbuff, Claude Code, Codex, or any custom TUI application
...
### Testing Openbuff (Default)

```bash
# Start a test session (defaults to Openbuff dev server)
...
```bash
# Build the binary first (Openbuff example)
cd cli && bun run build:binary

# Test with binary at default location (./cli/bin/openbuff)
...
CODEBUFF_BINARY=./cli/bin/openbuff ./scripts/tmux/tmux-cli.sh start
```
...
# Default settings (Openbuff dev server)
...
# Test a compiled binary (default location: ./cli/bin/openbuff)
...
- `@openbuff-tester` - Tests the Openbuff CLI
